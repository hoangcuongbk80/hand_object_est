...
def remove_invisible_hand_points(cloud, hand_points, pose, th=0.01):
    """ Remove invisible part of object model according to scene point cloud.

        Input:
            cloud: [np.ndarray, (N,3), np.float32]
                scene point cloud
            hand_points: [np.ndarray, (M,3), np.float32]
                hand point label in object coordinates
            pose: [np.ndarray, (4,4), np.float32]
                transformation matrix from object coordinates to world coordinates
            th: [float]
                if the minimum distance between a hand point and the scene points is greater than outlier, the point will be removed

        Output:
            visible_mask: [np.ndarray, (M,), np.bool]
                mask to show the visible part of hand points
    """
    hand_points_trans = transform_point_cloud(hand_points, pose)
    dists = compute_point_dists(hand_points_trans, cloud)
